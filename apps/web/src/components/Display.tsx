...
export default function Display({ budgets, currentBudget }: { budgets: IBudget[]; currentBudget: IBudget }) {
  ...
  const checkDeadline = (budget: IBudget) => {
    ...
    if (difference.getDate() > 0) {
      return (
        <div>
          <p className="font-medium p-4 mb-2">
            Your budget created on: {budgetDate.toDateString()} expires next month on: {monthAhead.toDateString()}
          </p>
  ...
  return (
    <div className="container mx-auto mt-8 border border-gray-400 rounded-md overflow-hidden sm:w-1/2 lg:w-3/4 h-5/6 mb-[40px]">
      ...
      <ul className="list-inside">
        <h2 className="text-lg font-medium p-4 mb-2">
          Total Budget: spent ${CurrentBudget?._current.toFixed(2)} out of ${CurrentBudget?._budget}
        </h2>
        <div className="p-4">
          <ProgressBar
            completed={Math.ceil((CurrentBudget?._current ? CurrentBudget._current / CurrentBudget._budget : 0) * 100)}
          />
        </div>
        {CurrentBudget?.expenses.map((category, key) => {
              ...
              <li key={category.category} className="p-4">
                <div className="flex flex-row">
                  <h3 className="text-lg font-medium mb-2">
                    {category.category}: spent ${category.current.toFixed(2)} out of ${category.budget}
                  </h3>
                </div>
